...
@implementation UIColor (ZB)

+ (instancetype)ZBColor:(id)value{
    if (!value || [value isKindOfClass:[NSNull class]]) {
        return nil;
    }
    static NSCache *colorCaches;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        colorCaches = [[NSCache alloc] init];
        colorCaches.countLimit = 64;
    });
    UIColor *color = [colorCaches objectForKey:value];
    if (color) {
        return color;
    }
    ...
    if ([value isKindOfClass:[NSString class]]) {
        ...
        if ([rgba hasPrefix:@"#"]) {
            ...
            int r,g,b,a;
            sscanf(rgba.UTF8String, "rgba(%d,%d,%d,%d)", &r,&g,&b,&a);
            red = r/255.0;
            green = g/255.0;
            blue = b/255.0;
            alpha = a;
    ...
    if (color && value) {
        [colorCaches setObject:color forKey:value];
    }
    return color;
}

+ (instancetype)ZBHex:(int)value{
    return [self ZBColor:@(value)];
}
